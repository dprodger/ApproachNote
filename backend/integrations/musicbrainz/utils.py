"""
MusicBrainz song-update helpers.

Domain orchestrations that combine MusicBrainz lookups with songs-table
writes: pull composer / Wikipedia URL / composed year off a work and patch
our row if it's missing.

The low-level HTTP client (MusicBrainzSearcher) lives in
integrations.musicbrainz.client — re-exported here so existing callers
that do `from integrations.musicbrainz.utils import MusicBrainzSearcher`
keep working.
"""
